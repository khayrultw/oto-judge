...
func RequireStarted(c *gin.Context) {
	...
	println("Problem ID:", problemIdStr)
	...
	println("Contest Start Time:", contest.StartTime.UTC().String())
	println("Current Time:", time.Now().UTC().String())
	if contest.StartTime.Time.After(time.Now()) {
		println("Contest has not started yet")

...
function MyContestSubmissions() {
  ...
  const { contestId } = useParams();
    ...
    const fetchSubmissions = async () => {
        ...
        try {
            const res = await repo.getMyContestSubmissions(contestId);
        ...
        const es = new EventSource(BASE_URL + `/contests/${contestId}/sse/my?q=${q}`)
        es.onmessage = (event) => {
        try {
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl mb-4 text-center">My Contest Submissions</h1>
...
export default MyContestSubmissions;
